...
async def getGlobalFirepower(final_df, category_filter, column_hyperlinks, url, base_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Retrieve all links
            category_links = []
            category_links = await getCategoryLinksWithGroup(soup)

            # If web scraping by category groups fails, try to attempt webscraping 'Overview' section
            if category_links == []:
                category_links = await getCategoryLinks(soup)
            ...
            for href, button_text in category_links:
                ...
                extracted_df = await fetch_and_extract_data(full_url, button_text, category_filter, column_hyperlinks)
...
async def fetch_and_extract_data(url, button_text, category_filter, column_hyperlinks):
    try:
        # Attempt to fetch the webpage content
        response = requests.get(url)
        # Check if the request was successful (status code 200)
...
async def getCategoryLinksWithGroup(soup):
...
async def getCategoryLinks(soup):
