...
@app.callback(
    ...
    [Input('create-graph-button', 'n_clicks'),
     Input('sort-direction-dropdown', 'value')],  # Add this input
    [State('datatable', 'derived_virtual_selected_rows'),
     State('graph-type-dropdown', 'value'),
     State('value-selector-dropdown', 'value')]
)
def update_graph(n_clicks, sort_direction, selected_rows, graph_type, value_column):  # Add sort_direction parameter
    ...
    # Check if the graph to be generated is a bar graph
    if graph_type == 'bar':
        # Sort the DataFrame based on the value_column and the sort direction
        sorted_df = filtered_df.sort_values(by=value_column, ascending=True if sort_direction == 'asc' else False)
        fig = px.bar(sorted_df, x='Country Name', y=value_column, title="Bar Graph")
    elif graph_type == 'pie':
