...
@app.callback(
    ...
    [State('datatable', 'derived_virtual_data'),  # Current view of the data
     State('datatable', 'derived_virtual_selected_rows'),  # Indices of selected rows
     State('graph-type-dropdown', 'value'),
     State('x-axis-selector-dropdown', 'value'),  # Axis selection
     State('y-axis-selector-dropdown', 'value')]  # Y-axis selection for aggregation
)
def update_graph(n_clicks, sort_direction, virtual_data, selected_rows, graph_type, x_axis_selection, y_axis_selection):
    ...
    # Reconstruct DataFrame from the current view of the data
    ...
    if x_axis_selection == 'Affiliation':
        # Group by Affiliation and aggregate y-axis values
        df_aggregated = df_filtered.groupby('Affiliation')[y_axis_selection].sum().reset_index()
    else:
        df_aggregated = df_filtered

    if graph_type == 'bar':
        is_ascending = sort_direction == 'asc'
        sorted_df = df_aggregated.sort_values(by=y_axis_selection, ascending=is_ascending)
        fig = px.bar(sorted_df, x=x_axis_selection, y=y_axis_selection, title="Bar Graph")
    elif graph_type == 'pie':
        # For pie charts, the values must aggregate to a total; this assumes aggregation is already suitable
        fig = px.pie(df_aggregated, names=x_axis_selection, values=y_axis_selection, title="Pie Chart")
    else:
