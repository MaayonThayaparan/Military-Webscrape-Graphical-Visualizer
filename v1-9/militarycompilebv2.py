...
def getGlobalFirepower(final_df, category_filter, column_hyperlinks):
...
def getNuclear(final_df, category_filter, column_hyperlinks):
...
def createTable():
    ...
    final_df = getNuclear(final_df, category_filter, column_hyperlinks)
    final_df = getGlobalFirepower(final_df, category_filter, column_hyperlinks)
    final_df[0].fillna(0, inplace=True)
...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        id='value-selector-dropdown',
        options=[{'label': col, 'value': col} for col in df.columns if col != 'Country Name'],
        value=df.columns[1],  # Assume the second column has numeric values for simplicity
        multi=False
    ),
...
@app.callback(
    ...
    [State('datatable', 'derived_virtual_selected_rows'),
     ...
     State('value-selector-dropdown', 'value')]
)
def update_graph(n_clicks, selected_rows, graph_type, value_column):
    ...
    if graph_type == 'bar':
        fig = px.bar(filtered_df, x='Country Name', y=value_column, title="Bar Graph")
    elif graph_type == 'pie':
        fig = px.pie(filtered_df, names='Country Name', values=value_column, title="Pie Chart")
    else:
        return html.Div('Please select valid parameters for the graph.')
