...
df, category_filter, column_hyperlinks = retrieve_data('2024')
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div(id='page-controls', className='col-md-8', children=[
...
# Update the callback to retrieve data from MongoDB when collection is selected
@app.callback(
    [Output('datatable', 'data'),
     Output('datatable', 'columns'),
     Output('group-selector-dropdown', 'options'),
     Output('group-selector-dropdown-multi', 'options'),
     Output('column-references-dropdown', 'options')],
    [Input('collection-selector-dropdown', 'value')]
)

def update_data_and_ui(selected_collection):
    if not selected_collection:
        raise PreventUpdate

    # Retrieve data from MongoDB and update DataFrame and dictionaries
    df, category_filter, column_hyperlinks = retrieve_data(selected_collection)
    ...
    # Dropdown for selecting group on table
    group_selector_dropdown = [{'label': group, 'value': group} for group in group_to_columns.keys()]
  

    # Dropdown for selecting group for multi-bar-graph
    group_selector_dropdown_multibar =[{'label': group, 'value': group} for group in group_to_columns.keys()]
    
    column_references_dropdown = [{'label': f'{name}: {link}', 'value': f'{name}||{link}'} for name, link in column_hyperlinks.items()]
                
    return data, columns, group_selector_dropdown, group_selector_dropdown_multibar, column_references_dropdown
